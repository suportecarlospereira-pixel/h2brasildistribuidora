...
export const AdminView: React.FC<AdminViewProps> = ({ allDrivers, allLocations, onDistributeRoutes }) => {
    ...
    const handleDeleteDriver = async (id: string, name: string) => {
        ...
        if (window.confirm(`ATENÇÃO: Deseja remover o motorista "${name}" do sistema?`)) {
            ...
            try {
                ...
                alert("Erro ao excluir motorista. Verifique permissões.");
            } finally {
                setDeletingId(null);
            }
        }
    ...
    const handleDeleteHistory = async (e: React.MouseEvent, historyId: string, driverName: string) => {
        e.stopPropagation(); // Impede abrir o accordion
    ...
    const pendingLocations = allLocations.filter(l => l.type !== 'HEADQUARTERS'); // String direta para evitar bug
                    ...
                    <div className="p-4 space-y-4">
                        <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm">
                            ...
                            <div className="flex flex-col gap-2">
                                <div className="flex items-center gap-2">
                                    <div className="flex-1 bg-slate-50 p-2 rounded-xl border border-slate-100 flex flex-col">
                                        ...
                                        <input 
                                            ...
                                            className="bg-transparent w-full text-slate-900 focus:outline-none font-bold text-xs uppercase" 
                                        />
                                    </div>
                                    <div className="flex-1 bg-slate-50 p-2 rounded-xl border border-slate-100 flex flex-col">
                                ...
                                <div className="text-[10px] text-center text-slate-400 font-medium mt-1">
                                ...
                                filteredHistory.map(record => {
                                    ...
                                    return (
                                        <div key={record.id} className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden relative group">
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    <div className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center font-black text-sm text-slate-600">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div className="flex gap-2 mt-1 flex-wrap">
                                                            ...
                                                            {record.totalFailures > 0 && (
                                                                <span className="text-[9px] font-bold bg-red-100 text-red-700 px-2 py-0.5 rounded">
                                            ...
                                            {isExpanded && (
                                                <div className="bg-slate-50 border-t border-slate-100 p-4 space-y-2">
                                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Detalhamento da Rota</p>
                                                    ...
                                                    {record.records && record.records.length > 0 ? (
                                                        ...
                                                        <p className="text-xs text-slate-400 text-center italic">Sem detalhes registrados.</p>
                                                    )}
                                                </div>
                                            )}
                                        </div>
